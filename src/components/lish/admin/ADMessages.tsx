...
export const ADMessages = ({ onNavigate: _ }: { onNavigate: (s: any) => void }) => {
  ...
  const { data: requests = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("service_requests").select("id,title,status,client_id,profiles!service_requests_client_id_fkey(full_name,email)").not("status", "in", '("rejected","cancelled")').order("created_at", { ascending: false });
      return data ?? [];
    },
  ...
  if (requests.length === 0) return (
    <div><SectionHeader title="Messages" subtitle="Chat with clients." />
      <div className="bg-white rounded-2xl border border-stone-200 p-14 text-center"><MessageSquare className="w-10 h-10 text-stone-300 mx-auto mb-3" /><p className="text-stone-400 text-sm">No active requests yet.</p></div>
    </div>
  );
    ...
    <div>
      ...
      <div className="flex gap-4 h-[calc(100vh-260px)] min-h-[420px]">
        <div className="w-52 shrink-0 space-y-1 overflow-y-auto">
          {requests.map((r: any) => (
            <button key={r.id} onClick={() => setSelectedReq(r.id)} className={`w-full text-left px-3 py-3 rounded-xl text-sm transition-all ${selectedReq === r.id ? "bg-rose-50 text-rose-600 border border-rose-100" : "text-stone-500 hover:bg-stone-50"}`}>
              <p className="truncate text-xs font-medium">{r.title}</p>
              <p className="text-[10px] mt-0.5 text-stone-400">{(r as any).profiles?.full_name || (r as any).profiles?.email || "Client"}</p>
            </button>
          ))}
        </div>
        <div className="flex-1 flex flex-col bg-white rounded-2xl border border-stone-200 overflow-hidden">
          <div className="px-4 py-3 border-b border-stone-100 bg-stone-50">
            ...
            <p className="text-xs text-stone-400">{(selected as any)?.profiles?.full_name || (selected as any)?.profiles?.email || "Client"}</p>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-3">
            {messages.length === 0 && <p className="text-center text-xs text-stone-400 py-8">No messages yet.</p>}
            {messages.map((m: any) => {
          ...
          <form onSubmit={send} className="p-3 border-t border-stone-100 flex gap-2">
            <Input value={msg} onChange={e => setMsg(e.target.value)} placeholder="Type a message..." className="rounded-full h-10 flex-1" />
            <Button type="submit" disabled={sending || !msg.trim()} size="icon" className="rounded-full w-10 h-10 bg-rose-500 text-white border-0 shrink-0 hover:bg-rose-600"><Send className="w-4 h-4" /></Button>
          </form>
        </div>
      </div>
    </div>
