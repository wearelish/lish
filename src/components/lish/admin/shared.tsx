...
export const KpiCard = ({ icon: Icon, label, value, sub, color = "rose" }: {
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 p-5 hover:shadow-sm transition-shadow">
      <div className={`w-9 h-9 rounded-xl ${colors[color] ?? colors.rose} flex items-center justify-center mb-3`}>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-stone-100 bg-stone-50">
            {headers.map(h => (
              <th key={h} className="text-left px-4 py-3 text-[11px] uppercase tracking-wider text-stone-400 font-semibold whitespace-nowrap">{h}</th>
            ))}
          </tr>
        </thead>
        <tbody>
          {empty ? (
            <tr><td colSpan={headers.length} className="px-4 py-12 text-center text-stone-400 text-sm">No records found.</td></tr>
          ) : children}
        </tbody>
      </table>
    </div>
...
export const TR = ({ children, onClick }: { children: React.ReactNode; onClick?: () => void }) => (
  <tr onClick={onClick} className={`border-b border-stone-50 last:border-0 hover:bg-stone-50 transition-colors ${onClick ? "cursor-pointer" : ""}`}>
    {children}
  </tr>
);
